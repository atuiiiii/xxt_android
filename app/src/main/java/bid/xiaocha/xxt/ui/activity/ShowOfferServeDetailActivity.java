...
public class ShowOfferServeDetailActivity extends AppCompatActivity implements IShowOfferServeDetailView, View.OnClickListener ,ICollectServeView{
    ...
    private CollectServePresenter collectServePresenter;
    ...
    private void initPresenter() {
        collectServePresenter = new CollectServePresenter(this);
    ...
    private void initView() {
        ...
        contentBinding.btnCollect.setOnClickListener(this);
    ...
    @Override
    public void isCollectResult(short isResult) {
        if(isResult == 0){
            isCollect = true;
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    contentBinding.btnCollect.setText("已收藏");
                }
            });
        }if (isResult == 4){
            isCollect = false;
    ...
    @Override
    public void showResult(short result) {
        if(isCollect) {
            switch (result) {
                case 0:showToast("收藏已取消");break;
                case 1:showToast("收藏取消失败");break;
                case 2:showToast("服务器故障");break;
                case 3:showToast("无收藏");break;
                case 4:showToast("网络故障");break;
            }
        }else{
            switch (result) {
                case 0:showToast("收藏成功");break;
                case 1:showToast("收藏失败");break;
                case 2:showToast("服务器故障");break;
                case 3:showToast("已收藏");break;
                case 4:showToast("网络故障");break;
            }
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.btn_collect:
                if(isCollect){
                    collectServePresenter.deleteCollect(UserEntity.getCurrentUser().getUserId(),offerServeEntity.getServeId());
                }else{
                    collectServePresenter.addOneCollect(UserEntity.getCurrentUser().getUserId(),offerServeEntity.getServeId());
                }
                break;
        }
    }
